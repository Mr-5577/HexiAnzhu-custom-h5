<template>
	<view class="uni-flex-center">
		<view class="margin-sm" v-if="customerData">
			<view class="text-black text-lg">成交信息</view>
			<view class="margin-top border bg-gray text-15 radius">
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">项目名称</text>
					<text class="">{{customerData.pactData.p_name}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">房间号码</text>
					<text class="">{{customerData.pactData.roomnum}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">客户名称</text>
					<text class="">{{customerData.pactData.cname}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">建筑面积</text>
					<text class="">{{customerData.pactData.buildareas}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">联系电话</text>
					<text class="">{{customerData.pactData.ctel}}</text>
				</view>
				<view class="flex justify-between padding-sm  " v-if="customerData.pactData.ctels">
					<text class="text-gray">备用电话</text>
					<text class="">{{customerData.pactData.ctels}}</text>
				</view>
				
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">身份证</text>
					<text class="">{{customerData.pactData.cidcard}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">成交总价</text>
					<text class="">{{ nToLocalString(customerData.pactData.allmoney) }}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">成交时间</text>
					<text class="">{{customerData.pactData.dtime}}</text>
				</view>
				<view class="flex justify-between padding-sm  ">
					<text class="text-gray">置业顾问</text>
					<text class="">{{customerData.pactData.u_name}}</text>
				</view>
			</view>
			<!-- //付款信息"payData": -->
			<view class="margin-top">
				<view class="text-black text-lg">付款信息</view>
			</view>
			<view class="margin-top">
			...
			<!-- //逾期"paymentData": [], -->
			<view class="margin-top">
				<view class="text-black text-lg">逾期信息</view>
			</view>
			<view class="margin-top">
				<t-table @change="change" :borderColor="borderColor">
					<t-tr>
			...
			<!-- //催收记录"dunData": [] -->
			<view class="margin-top">
				<view class="text-black text-lg">催收记录</view>
			</view>
			<view class="margin-top">
		...
		<view class="uni-common-mark flex justify-center align-center" v-if="showDun" @click="showDun=false" >
			<view class=" uni-font-size24 test " @tap.stop="stop">
				<textarea placeholder="请填写催办记录" class="bg-gray padding" maxlength="-1" v-model="dunDatas.dun_reason"></textarea>
				<button @click="addDunning" class=" round">确定添加</button>
			</view>
		</view>
	</view>
</template>
