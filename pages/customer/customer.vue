		...
		<template>
			...
			<view class="cu-list menu-avatar">
				<view class="cu-item shadow-gray" v-for="(items, index) in customerList" :key="index" @click="detail(items)">
					<view class="padding-left-sm flex justify-between padding-right-sm item-cell-list">
						<view class="flex">
					...
					<view class="padding-top  padding-left-sm flex ">
						<view class="cu-avatar round lg bg-blue text-white ">{{ cName(items.cname) }}</view>
						<view class="uni-cell-90 padding-left-sm">
...
export default {
	...
	onShow() {
		...
		//是否有3天以上没有跟进筛选
		this.requestData.record = this.records;
		if (dap.length == 0) {
			this.getUser();
		} else if (this.ChooseProject != dap) {
		...
		if (this.types != this.requestData.type && this.types == 1) {
	...
	methods: {
		...
		tabSelect(item, index) {
			...
			//设置默认项目
			this.$api.SetDefaultpidbookingapi({ pid: this.requestData.pid }, res => {
				if (res.code != 1000) {
					uni.showToast({
						icon: 'none',
						title: res.msg
